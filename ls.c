 ...
 *                                                                      *
 *        20-Aug-2022:                                                  *
 *        Output adjusted to terminal size of 64x32 characters          *
 *                                                                      *
 ************************************************************************/
...
char **argv;             /* argument vector */
{
  ...
  if ((count = save_fcb(fcb_ptr,attr_sw,&tot,got_pattern)) == -1){
    ...
    if (sort_sw) sort_fcb(fcb_ptr,count,11);
...
int got_pattern;
{
  ...
  cusr = usr;
  *ncnt = nsave = 0;
  if ((n = bdos(17,FCBADR)) == 255) return(0);
  do {
    (*ncnt)++;
    buf_ptr = DMABUF+32*n;
    sys = *(buf_ptr+10) & MASK;
    ...
    if (((attr == 1 &&  sys) || (attr == 2 && ro) ||
      ...
      if ((p = malloc(SSIZE)) == NULL) return(-1);
      copy_fcb(p, buf_ptr);
      fcb_ptr[nsave++] = p;
    }
...
int count, tot;
{
  ...
  for (i = 0; i < count; i++) {
    ...
    switch (size) {
      case 0:
        printf("%s%s", name, (i%4==3 || i==count-1) ? "\n" : " | ");
        break;
      case 1:
        printf("%s%5u", name, n);
        printf("%s", (i%3==2 || i==count-1) ? "\n" : " | ");
        break;
      case 2:
        printf("%s%5u sectors %4dK", name, n, s);
  ...
  /* printf("   %u/%u Blocks used",tblks, ksize*8);   TL;DR */
  ...
  for (gap = n/2; gap > 0; gap /= 2)
    for (i = gap; i < n; i++)
      for (j = i-gap; j >=0; j -= gap) {
        if (sstrcmp(v[j], v[j+gap], num) <= 0) break;
        temp = v[j];
        v[j] = v[j+gap];
        v[j+gap] = temp;
      }
...
char *ptr;
{
  ...
  ex = *(ptr+12);
  n = *(ptr+15);
  if (ex != 0 && n != 128) n = ex*128+n;
  else {
    if (n == 128) {
      if ((p = malloc(SSIZE+20)) == NULL) return(0);
      copy_fcb(p, ptr);
      bdos(35,p);
      tmptr = p+33;
      n = *tmptr;
